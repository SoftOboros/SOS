...
- (a) ⏸ PCDN-SOS-09-D-001 through 005 resolved (§15).
- (b) ⏸ The SOS-09-D walker presence — `tools/sos-codegen/transliterate_rust.py` (existing) carries the SOS-09-D-channel-emit subset and emits `RegisterBlock` + newtype family + type-state-for-shared + MPU constants per §5.
- (c) ⏸ The `RegisterBlock` layout gate (§9 (a)) is verified for at least one worked-example chart.
- (d) ⏸ The newtype family completeness gate (§9 (b)) is verified for at least one channel per `sos:kind` value (`status`, `command`, `queue`, `shared`) plus at least one channel per side-effect/clear-on-read variant (`ClearOnRead`, `FireOnWrite`).
- (e) ⏸ The type-state-for-shared gate (§9 (c)) is verified by a `compile-fail/` test against the emitted crate.
- (f) ⏸ The `cargo check` clean gate (§9 (d)) is verified for the worked-example crate against `thumbv7em-none-eabihf`.
- (g) ⏸ The `*_unchecked` discharge gate (§9 (e)) is verified for at least one `*_unchecked` accessor with a chart-bounds-discharged invariant (the worked-example chart MUST exercise this path or document why no `*_unchecked` accessor is appropriate).
- (h) ⏸ The MPU-region constant export gate (§9 (f)) is verified for at least one channel with a chart-declared `sos:zone` or `sos:mpu_attr`.
- (i) ⏸ The `sos:name`-determinism gate (§9 (i)) is verified by a fixture pair.
- (j) ⏸ Cross-sub-phase invariants INV-S-MEM-D-1 through 6 cited correctly in the SOS-09-D emit-path source (typically via `@spec` comment block referencing this doc + SOS-07 + SOS-09 + SOS-09-A + SOS-09-B + SOS-09-G).
- (k) ⏸ SOS-09 umbrella INV-S-MEM-1 through 6 satisfied (the emitted crate is a build output per INV-S-MEM-2; single-source per INV-S-MEM-1; determinism per INV-SOS-G mirrors here as INV-S-MEM-D-5 / D-6).

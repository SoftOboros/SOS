...
Implementation wave-1 against the SOS-08-A primitive surface surfaced eight further ambiguities that the ratified-2026-05-23 surface did not pin down at sufficient precision for code generation. The PCDN walkthrough on the same date resolved all eight. These are continuation amendments — SOS-08-A is still ratified; this entry expands the normative surface to cover implementation choices that emerged during wave-1.

- **PCDN-A-fifo-READ_LATENCY → RESOLVED**: `sos_fifo_sync` ships with BOTH FWFT (`READ_LATENCY=0`) and registered-read (`READ_LATENCY=1`) variants via a mandatory `READ_LATENCY` generic. Both paths live in source — neither is the canonical-only path. The SVA `a_tdata_stable_until_tready` property is conditional on `READ_LATENCY=0` (registered-read breaks the across-cycle stability claim by design). Code amendment lands in the companion implementation commit.

- **PCDN-A-fifo-count → RESOLVED**: `count` stays as a **required** observability port on `sos_fifo_sync` at width `ceil(log2(DEPTH+1))` bits. It drives downstream backpressure logic and is the witness for the `count_invariant` SVA property in §6.2. §6.2's port enumeration currently omits `count` from the textual list (only the `count_invariant` SVA cites it); this amendment makes `count` normatively part of the port surface — see by-reference §6.2 amendment below.

- **PCDN-A-fifo-RESET_MEM → RESOLVED**: `sos_fifo_sync` gains a mandatory `RESET_MEM` generic (boolean). `false`/`0` preserves the current behaviour (mem array untouched on reset, only pointers/count clear); `true`/`1` clears mem to all-zero on reset (larger reset fanout, stricter post-reset semantics). The caller opts in based on whether their downstream consumer can tolerate stale-data reads in the post-reset window. Code amendment lands in the companion implementation commit.

- **PCDN-A-bind-form → RESOLVED**: SVA bind directive form across all SOS-08-A primitives is **module-type bind** (`bind <module> <sva-module> u_sva (.*);`), attaching the assertion module to every instance of the primitive across the design — not per-instance bind. Module-type is Verilator-compatible, while per-instance bind has uneven Verilator support. Frozen for all eleven L0 primitives; SOS-08-D's emitter MUST emit this form.

- **PCDN-A-mutex-ack → RESOLVED**: `sos_mutex`'s `ack[i]` is **level-held** while client `i` holds the lock — asserted every cycle of ownership, not just the acquire pulse. The invariant `locked == |ack` holds across every cycle. This diverges from §5.1's literal "1-cycle pulse ack" wording; §5.1 is amended below to recognize **two control-handshake variants for stateful primitives**: (a) **pulse-based** for one-shot acknowledgements (`sos_strobe_latch`'s `ack`, `sos_arbiter_rr`'s request/grant pair, `sos_credit_counter`'s consume/replenish acks), (b) **level-held** for ownership-tracking primitives (`sos_mutex`, and any future stateful-lease primitives where the ack encodes continuous ownership rather than a transient event). The frozen-enumeration registration policy for the two-variant set is **Standards Action**.

- **PCDN-A-mutex-fairness → RESOLVED**: `sos_mutex`'s `fairness_rr` SVA property uses a **best-effort form** with an escape hatch: `req && !ack |-> ##[1:N_CLIENTS] ack || !req`. The trailing `|| !req` clause lets a requester rescind before the round-robin window closes without failing the assertion. The unbounded-hold case (a holder never releasing) still depends on chart-side `CHART_BOUND` emission from SOS-08-C; SOS-08-A v1 ships the escape-hatched form. Tightening to a strict `|-> ##[1:N_CLIENTS] ack` lands once `CHART_BOUND` plumbing exists in SOS-08-C emission.

- **PCDN-A-mutex-N_CLIENTS-min → RESOLVED**: `sos_mutex` adds a static elaboration-time assertion in **both** VHDL (`assert N_CLIENTS >= 2 report "..." severity failure;` in the entity body) and SystemVerilog (`if (N_CLIENTS < 2) $fatal(1, "...");` in an `initial` or generate-time check) enforcing `N_CLIENTS >= 2`. §6.5's "`N_REQUESTERS ≥ 2`" guidance is thus normatively enforced — under-instantiation fails synth, not silently at runtime. Code amendment lands in the companion implementation commit.

- **PCDN-A-arbiter-GRANT_LATENCY_CYCLES → RESOLVED**: `sos_arbiter_rr` adds a `GRANT_LATENCY_CYCLES` generic (values `0` | `1`, **default `1`**). `=1` is the canonical registered-grant shape (grant asserts one cycle after the winning `req`); `=0` is the combinational-grant opt-in variant (grant tracks `req` within the same cycle, lower latency, higher critical-path cost). The SVA `eventually_granted` bound widens to `##[1:N_REQS+GRANT_LATENCY_CYCLES]` to cover both shapes. **This is the ONE generic in the entire SOS-08-A surface that receives a default value** — rationale: the registered shape is the canonical primitive surface (every other instantiation site wants `=1`), and the combinational variant is a rare opt-in for low-latency arbitration paths where the user has audited the critical-path cost. INV-S-HDL-A-5 (mandatory parameters have no defaults) is amended below with a named exception for this single generic.

**§5 + INV amendments**:

- **§5.1 (interface-shape conventions)** wording extended to recognize **two control-handshake variants for stateful primitives** per PCDN-A-mutex-ack: (a) pulse-based ack for one-shot events, (b) level-held ack for ownership-tracking primitives. The single "control: req/ack (1-cycle pulses)" row is replaced by two rows distinguishing pulse-based from level-held ack semantics, with the primitive-set membership listed for each. By-reference amendment — the §15 entry is the canonical record; future commits MAY inline the amended row text into §5.1 itself.

- **INV-S-HDL-A-5 (mandatory parameters have no defaults)** wording extended with a **named exception for `GRANT_LATENCY_CYCLES`** per PCDN-A-arbiter-GRANT_LATENCY_CYCLES. The invariant now reads (in effect): "depth/width/count parameters that determine resource cost have no defaults; the sole exception is `GRANT_LATENCY_CYCLES` on `sos_arbiter_rr`, which defaults to `1` (the canonical registered-grant shape)." No other defaults are introduced into the SOS-08-A primitive surface by this amendment. By-reference amendment — recorded here; future commits MAY inline into §7.

- **§6.2 (`sos_fifo_sync`)** port list amends to **add `count`** (width `ceil(log2(DEPTH+1))`) per PCDN-A-fifo-count, and the generics list amends to add `READ_LATENCY` (per PCDN-A-fifo-READ_LATENCY) and `RESET_MEM` (per PCDN-A-fifo-RESET_MEM). By-reference amendment — recorded here; the §6.2 prose is NOT re-edited inline in this PR. Future commits MAY inline the amended port/generic enumeration into §6.2's body.

- **§6.5 (`sos_mutex`)** port set **adopts the collapsed `req` / `ack` / `locked` / `holder_id` surface** validated by the wave-1 implementation. The original four-port surface (`acquire_req` / `acquire_ack` / `release_req` / `release_ack`) drafted in §6.5 is **withdrawn**; the collapsed surface is the normative form going forward. Acquire and release semantics are encoded by the ownership-tracking level-held `ack` (per PCDN-A-mutex-ack) rather than two separate pulse pairs — when a client deasserts `req` while holding `ack`, that IS the release. By-reference amendment.

- **§6.3 (`sos_arbiter_rr`)** port set **confirms the collapsed `req` / `grant` / `last_winner_id` surface** from impl wave-1. The spec's draft `N_GRANTS` parameter + `grant_valid[N_GRANTS-1:0]` array are **dropped** as wave-1 implementation reality — multi-grant-per-cycle is not in scope for the first-target ECP5 deployment, and the single-grant shape composes cleanly without it. If multi-grant arbitration is needed later, a separate primitive `sos_arbiter_rr_multi` is the right shape rather than re-extending the canonical single-grant primitive. `last_winner_id` is added as an observability port for downstream debug. By-reference amendment.

**Status**: 🟢 **ratified (continuing)** — impl wave-1 PCDN amendments folded into the SOS-08-A normative surface. SOS-08-A is still ratified; this entry expands the surface to cover the implementation choices that emerged during wave-1. The top-of-file 🟢 ratified badge is unchanged (this is a continuation amendment, not a re-ratification). Code commits implementing the `READ_LATENCY` / `RESET_MEM` / `N_CLIENTS>=2` static assertion / `GRANT_LATENCY_CYCLES` choices land as companion implementation commits.

### 2026-05-23 — Impl wave-2 PCDN amendments (Ira)

Implementation wave-2 extended the L0 primitive catalogue beyond the wave-1 set (`sos_fifo_sync`, `sos_mutex`, `sos_arbiter_rr`) to cover the remaining eight primitives (`sos_fifo_async`, `sos_arbiter_priority`, `sos_credit_counter`, `sos_dpram_arb`, `sos_tick_gen`, `sos_synchronizer`, `sos_strobe_latch`, `sos_rate_divider`). All eleven L0 primitives now have ratified ports + generics + SVA + cocotb shape. As with wave-1, this is a continuation amendment — SOS-08-A is still ratified; this entry expands the normative surface to cover the implementation choices that emerged during wave-2.

Twelve user-resolved PCDNs from the wave-2 walkthrough:

- **PCDN-A-arbiter-GRANT_LATENCY_CYCLES-extension → RESOLVED**: INV-S-HDL-A-5's named exception for `GRANT_LATENCY_CYCLES` (introduced in wave-1 for `sos_arbiter_rr`) is **extended by category** to cover **all L0 arbiter primitives** — currently `sos_arbiter_rr` + `sos_arbiter_priority`. Future arbiter primitives inherit the exception by category, not by name. The invariant's exception clause now reads (in effect): "the sole exceptions are `GRANT_LATENCY_CYCLES` on arbiter primitives (registered-grant canonical shape) and `SYNC_STAGES` on CDC primitives (see PCDN-A-dpram-SYNC_STAGES below)."

- **PCDN-A-counter-prose-collapse (§6.6) → RESOLVED**: Spec body §6.6's drafted `consume_req` / `consume_ack` / `replenish_req` / `replenish_ack` / `count` / `empty` / `full` port surface is **WITHDRAWN**. The canonical normative surface is `acquire_req` / `acquire_ack` / `release_req` / `credits` per the wave-1 impl validated against `sos_mutex` (matching the §6.5 by-reference collapse pattern from the wave-1 entry). §6.6's drafted prose stays as-is for archival continuity; this §15 entry is the **canonical record** of the port surface. By-reference amendment.

- **PCDN-A-tick-prose-collapse (§6.8) → RESOLVED**: Spec body §6.8's drafted 32-bit monotonic `tick_count[31:0]` observability port is **WITHDRAWN**. The canonical observability is a modulo `counter` port of width `clog2(PERIOD_CYCLES)` per the wave-2 impl. Charts wanting a monotonic tick count compose an external counter driven by the `tick` output. The 32-bit monotonic form added 32 bits of register state per tick generator for a feature that virtually every consumer either doesn't need or wants at a different width — pushing it outside the primitive is correct. By-reference amendment.

- **PCDN-A-rate-prose-collapse (§6.11) → RESOLVED**: Spec body §6.11's drafted runtime `ratio` input + `MAX_RATIO` generic surface is **WITHDRAWN**. The canonical surface is compile-time `DIVISOR` + `INITIAL_COUNTER` generics per the wave-2 impl. Charts wanting runtime variation compose multiple dividers with a mux on `tick_out`. Rationale: runtime ratio adjustment is rare in practice; the compile-time form is smaller (no comparator against a runtime register) and the mux-of-dividers pattern is explicit at the chart level. By-reference amendment.

- **PCDN-A-async-DEPTH-pow2 → RESOLVED**: `sos_fifo_async`'s `DEPTH` generic MUST be a power of two AND `>= 4`, enforced via elaboration-time `assert` in both VHDL and SV. §6.1's drafted "power of 2, ≥ 4" guidance was informative; this PCDN makes it normatively enforced — under-instantiation fails at elaboration, not silently at runtime. Code amendment lands in companion implementation commit.

- **PCDN-A-async-asymmetric-reset → RESOLVED**: `sos_fifo_async` deployments SHOULD tie `wr_rst` and `rd_rst` together at the system level. Asymmetric reset (one side reset, the other not) enters a degenerate state until `SYNC_STAGES` cycles after the second reset releases, during which Gray-coded pointer crossings observe stale state. This is the caller's responsibility per the wave-2 impl; spec §6.1 SHOULD carry this in normative text via this §15 entry's by-reference amendment.

- **PCDN-A-dpram-port-naming → RESOLVED**: Sub-variant of PCDN-A-002 for symmetric multi-port data primitives. `sos_dpram_arb` uses `port_a_*` / `port_b_*` prefix instead of `m_axis_t*` / `s_axis_t*` because DPRAM has no master/slave direction — both ports are peers. Future symmetric multi-port data primitives use the same pattern. Recorded as `PCDN-A-002-variant-symmetric-multi-port`. The wave-1 PCDN-A-002 resolution ("AXI-Stream prefixes on data-bearing primitives") is narrowed: it applies to **directional** data primitives (FIFOs); symmetric multi-port data primitives use the `port_<letter>_*` form.

- **PCDN-A-dpram-SYNC_STAGES → RESOLVED**: `sos_dpram_arb` adds a `SYNC_STAGES` generic (mandatory-with-default-2 per the CDC-primitive convention). INV-S-HDL-A-5's named-exception clause is **extended** to cover `SYNC_STAGES` on CDC primitives — currently `sos_fifo_async`, `sos_synchronizer`, and `sos_dpram_arb` in dual-clock mode. The exception list in INV-S-HDL-A-5 now reads: (a) `GRANT_LATENCY_CYCLES` on arbiter primitives (per PCDN-A-arbiter-GRANT_LATENCY_CYCLES-extension above); (b) `SYNC_STAGES` on CDC primitives (this PCDN). Code amendment lands in companion implementation commit.

- **PCDN-A-counter-release-at-max → RESOLVED**: `sos_credit_counter` `release_req` at `credits == MAX_CREDITS` is a **silent no-op**. No error signal is emitted. The chart compiler enforces release-at-max correctness via bounded reachability per INV-SOS-G (cited in SOS-07 §6) — if a chart's bounded execution reaches a release-at-max state, the compiler flags it; no runtime witness is needed inside the primitive. Keeps the primitive's port surface narrow.

- **PCDN-A-counter-same-cycle-edges → RESOLVED**: Same-cycle `acquire_req` + `release_req` on `sos_credit_counter` produces:
  - **At interior** (`0 < credits < MAX_CREDITS`): net-neutral — `credits` unchanged on the next cycle; `acquire_ack` pulses (the release "supplies" the acquire within the same cycle).
  - **At `credits == 0`**: release succeeds (`credits == 1` next cycle); acquire fails (no `acquire_ack`).
  - **At `credits == MAX_CREDITS`**: acquire succeeds (`credits` decrements next cycle); release is silently dropped per PCDN-A-counter-release-at-max.
  The SVA property `p_same_cycle_net_neutral` is conditional on `0 < credits < MAX_CREDITS` per the wave-2 impl — the boundary cases (`credits == 0` and `credits == MAX_CREDITS`) are not net-neutral and have their own conditional properties (`p_same_cycle_at_zero`, `p_same_cycle_at_max`).

- **PCDN-A-strobe-pending-shadow → RESOLVED**: `sos_strobe_latch` gains a `pending_strobe` shadow register (depth-1). The internal FSM expands from a 1-bit state (IDLE / LATCHED) to a 3-state one-hot FSM (IDLE / LATCHED / LATCHED_PENDING). A new port `pending_q` exposes the shadow register state for observability. The strobe that arrives same-cycle with `ack`-from-LATCHED is now **captured into the shadow** (previously dropped per the wave-1 ack-wins-same-cycle resolution); on the next cycle the FSM transitions LATCHED_PENDING → LATCHED with the shadow strobe consumed, preserving the ack-wins-same-cycle guarantee while eliminating the strobe-loss case that motivated this PCDN. INV-S-HDL-A-4's "trivial one-hot" wording is **extended** to "small one-hot (2-4 states)" to accommodate this primitive's 3-state FSM as the worked example. By-reference amendment.

- **PCDN-A-strobe-reset-value → RESOLVED (DEFERRED)**: `sos_strobe_latch`'s proposed `RESET_VALUE` generic (initial latched-state on reset release) is **deferred** — not added in v1. Spec §6.10 stays parameterless. Future addition would require a new §15 amendment + would count as another INV-S-HDL-A-5 named exception (since `RESET_VALUE` is mandatory-with-default by nature). The current v1 reset-clears-latch behaviour is canonical until that future amendment lands.

**Agent-default ratifications**:

Twelve wave-2 design choices that the user did not override at walkthrough; the §15 entry stamps them ratified without re-ratification overhead:

- **`sos_fifo_async`**: per-domain conservative `wr_count` / `rd_count` view (count semantics under CDC skew: write-side count is upper-bound on outstanding entries from the write-domain perspective, read-side count is lower-bound); cocotb-sequence-comparison ordering check shape (token-tagged write, monotonic-tag-delivery read-side assertion).
- **`sos_arbiter_priority`**: higher-priority-value-wins direction (i.e. `priority[i] = 3'b111` beats `priority[i] = 3'b000`); shared round-robin pointer within priority lane (ties among equal-priority requesters resolved by round-robin pointer, not by index); `aged_flag` MSB promotion encoding `{aged_flag, priority_in[i]}` — effective priority is the concatenation of the aging flag and the input priority, ensuring aged requesters always win against non-aged peers at the same input priority.
- **`sos_dpram_arb`**: A-wins collision priority on simultaneous A+B writes to same address (port-A wins by convention; chart-emitted designs that want symmetric handling compose two A-wins instances rather than encode tie-breaking inside the primitive); "read-old" semantic for same-cycle write+read on the same address (the read observes the pre-write value, matching standard BRAM behaviour).
- **`sos_tick_gen`**: combinational pulse-width-1 form (`tick` is asserted combinationally on the cycle the counter rolls over, not registered through an additional flop); `PERIOD_CYCLES >= 2` (PERIOD_CYCLES=1 rejected at elaboration — a tick-every-cycle is a degenerate case that should be encoded as `assign tick = enable` at the chart level); `period_stable` SVA tightened to `$rose(tick) |-> enable throughout (!tick [*PERIOD_CYCLES-1])` form (replaces the §6.8 drafted `##PERIOD_CYCLES $rose(tick)` form, which did not constrain the in-between cycles to also have `!tick`).
- **`sos_synchronizer`**: SVA ships **functional/structural** properties only (`chain_propagates`, `reset_clears_chain`, `dst_follows_last_ff`) — no MTBF assertion (the MTBF claim is physical, recorded in `rtl/sos_synchronizer/MTBF.md` with `// VERIFIED_BY_SDC` annotation + sign-off, per PCDN-A-006 from the ratification entry). `STAGES` is **mandatory no-default** per INV-S-HDL-A-5 (§6.9's drafted "default 2" is informative only; the normative form is mandatory). `src_clk` is NOT a port on the synchronizer — by definition the source clock is asynchronous to the destination, and naming it as a port would falsely imply the synchronizer can guarantee anything about source-domain timing.
- **`sos_strobe_latch`**: ack-wins-same-cycle from LATCHED (the original wave-1 ratified choice; the `pending_strobe` shadow per PCDN-A-strobe-pending-shadow above ENRICHES this — when `strobe` arrives same-cycle with `ack`-from-LATCHED, the ack still wins for the current cycle, AND the strobe is captured into the shadow for the next cycle, so neither edge is dropped).
- **`sos_rate_divider`**: `tick_out_spacing_in_ticks` SVA reformulation (replaces the §6.11 drafted `ratio_stable` property — which referenced the dropped runtime `ratio` input; the new property asserts that the spacing between consecutive `tick_out` rises equals `DIVISOR` `tick_in` events); `DIVISOR=0` forbidden at elaboration (the §6.11 drafted "`ratio == 0` disables `tick_out`" semantic is dropped along with the runtime ratio input — disabling a rate divider is encoded by gating its `tick_in` at the chart level).

**§5 / §6 / §7 amendments (by-reference)**:

- **§5.1 (interface-shape conventions)** — **sub-PCDN-A-002 variant** added: symmetric multi-port data primitives use `port_<letter>_*` (e.g. `port_a_*` / `port_b_*`) instead of AXI-Stream `m_axis_*` / `s_axis_*` (per PCDN-A-dpram-port-naming). The wave-1 PCDN-A-002 resolution applies to directional data primitives (FIFOs); symmetric multi-port data primitives use the new variant. By-reference amendment.
- **INV-S-HDL-A-5** — named-exception list is now: (a) `GRANT_LATENCY_CYCLES` on arbiter primitives (per PCDN-A-arbiter-GRANT_LATENCY_CYCLES-extension); (b) `SYNC_STAGES` on CDC primitives (per PCDN-A-dpram-SYNC_STAGES). Replaces the wave-1 single-exception clause for `GRANT_LATENCY_CYCLES` on `sos_arbiter_rr`. By-reference amendment.
- **INV-S-HDL-A-4** — "trivial one-hot" wording extended to "small one-hot (2-4 states)" per PCDN-A-strobe-pending-shadow. The `sos_strobe_latch` 3-state FSM (IDLE / LATCHED / LATCHED_PENDING) is the worked example. By-reference amendment.
- **§6.1 (`sos_fifo_async`)** — `DEPTH=pow2` normatively enforced at elaboration (per PCDN-A-async-DEPTH-pow2); asymmetric reset is the caller's responsibility (per PCDN-A-async-asymmetric-reset). By-reference amendment.
- **§6.6 (`sos_credit_counter`)** — port-name surface collapsed to `acquire_req` / `acquire_ack` / `release_req` / `credits` (per PCDN-A-counter-prose-collapse); release-at-`MAX_CREDITS` is silent no-op (per PCDN-A-counter-release-at-max); same-cycle edge semantics piecewise-defined by `credits` state (per PCDN-A-counter-same-cycle-edges). By-reference amendment.
- **§6.7 (`sos_dpram_arb`)** — `port_a_*` / `port_b_*` naming (per PCDN-A-dpram-port-naming); `SYNC_STAGES` generic added (per PCDN-A-dpram-SYNC_STAGES); A-wins collision; read-old same-cycle semantics. By-reference amendment.
- **§6.8 (`sos_tick_gen`)** — port collapse to modulo `counter` of width `clog2(PERIOD_CYCLES)` (per PCDN-A-tick-prose-collapse); combinational pulse-width-1 form; `PERIOD_CYCLES >= 2` enforced at elaboration. By-reference amendment.
- **§6.9 (`sos_synchronizer`)** — `STAGES` mandatory no-default (overrides §6.9's drafted "default 2"); no `src_clk` port; SVA scope clarified to functional/structural only (MTBF is physical, sign-off-based per PCDN-A-006). By-reference amendment.
- **§6.10 (`sos_strobe_latch`)** — `pending_strobe` shadow register added; FSM grows to 3 states (IDLE / LATCHED / LATCHED_PENDING); new `pending_q` observability port (per PCDN-A-strobe-pending-shadow); `RESET_VALUE` generic deferred to a future amendment (per PCDN-A-strobe-reset-value). By-reference amendment.
- **§6.11 (`sos_rate_divider`)** — port collapse to compile-time `DIVISOR` + `INITIAL_COUNTER` generics, no runtime `ratio` input (per PCDN-A-rate-prose-collapse); `DIVISOR=0` forbidden at elaboration. By-reference amendment.

(All §5 / §6 / §7 amendments above are by-reference — this §15 entry IS the canonical record. The §5 / §6 / §7 prose is NOT re-edited inline in this PR. Future commits MAY inline the amended text into the bodies of those sections; until they do, this entry is the authoritative form.)

**Status**: 🟢 **ratified (continuing)** — impl wave-2 PCDN amendments fold the full L0 primitive catalogue's wave-2 design choices into the SOS-08-A normative surface. All 11 L0 primitives have ratified ports + generics + SVA + cocotb shape. The next layer (SOS-08-B service composition) can begin emitting against this frozen primitive surface. The top-of-file 🟢 ratified badge is unchanged (this is a continuation amendment, not a re-ratification).

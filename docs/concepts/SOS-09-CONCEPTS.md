...
**Status:** 🟢 **ratified 2026-05-23; amended 2026-05-25 (PCDN-001 → `other_attributes`); amended 2026-05-26 (PCDN-007 two-axis channel-group)**. All 7 PCDNs walked; resolutions recorded in §15; amendments in §16.
...
### 15.7 PCDN-SOS-09-007 — Channel-group as one axis or two

**Problem statement.** Walking the SOS-09-C/D/E/F PCDN round on 2026-05-26 surfaced a cross-cutting concern that did not fit cleanly in any single sub-phase: PCDN-SOS-09-D-001 (Rust `RegisterBlock` borrow-enclosure unit) and PCDN-SOS-09-E-003 (HDL access-violation strobe-latch aggregation unit) are both implicitly making the channel-group the right unit for THEIR respective concerns — memory-safety enclosure on the Rust side; privilege / access-violation aggregation on the HDL side. If those two backends ever need to diverge (per-register MPU + channel-group borrow semantics, or per-register borrow + channel-group MPU), collapsing them into a single annotation key forces a future migration through both backends simultaneously. The PCDN names the axis-count question explicitly so the chart-author surface chooses now rather than later.

**Options considered.**

- **(a) One axis** — a single `sos:channel_group` annotation key serves both backends. Simplest chart-author surface; lowest annotation noise. Risk: any future divergence between Rust-borrow scope and HDL-MPU scope forces a coordinated migration through both backends and the chart-author surface simultaneously.
- **(b) Two axes with default-from-inheritance** — two annotation keys, `sos:channel_group` (Rust borrow scope / `RegisterBlock` boundary) and `sos:privilege_region` (HDL MPU + access-violation aggregation), with `sos:privilege_region` inheriting from `sos:channel_group` when omitted. Common case stays one annotation; divergence is a chart-author edit, not a spec migration.
- **(c) Defer to v2** — leave the channel-group axis implicit at v1; revisit when concrete divergence pressure shows up. Risk: divergence pressure tends to surface as a fait accompli (one backend ships a workaround); the spec catches up after the fact, with the usual cost.

**Resolution: 🟢 RATIFIED 2026-05-26 — accepted option (b)**: two axes with default-from-inheritance. The annotation surface gains `sos:channel_group` (Rust borrow scope / `RegisterBlock` boundary) and `sos:privilege_region` (HDL MPU + access-violation aggregation) as new SOS-semantic keys within the `other_attributes` JSON map. Default rule: when `sos:privilege_region` is omitted, it inherits the value of `sos:channel_group`. The common case is one axis (chart author writes `sos:channel_group` only); divergence between Rust borrow semantics and HDL MPU semantics becomes a chart-author edit (set `sos:privilege_region` explicitly), NOT a spec migration through D and E simultaneously.

**Implementation note.** The actual annotation-key additions to SOS-09-A's §5.2 ten-key set (expanding it to a twelve-key set with `sos:channel_group` and `sos:privilege_region`) land JIT in a future SOS-09-A §15 amendment that ships with the SOS-09-D and SOS-09-E implementation PRs that consume the keys. Per the Spec-Before-Code discipline ("Touching a frozen enum value or an invariant requires a §15 amendment first, in a separate PR"), the SOS-09-A amendment lands as a separate small commit BEFORE either D or E implementation lands its parser / consumer code. This sequencing prevents the failure mode where one backend ships its consumer of the key before the chart-author surface is amended to permit the key.

**Cross-references.** PCDN-SOS-09-D-001 (Rust `RegisterBlock` borrow-enclosure unit) and PCDN-SOS-09-E-003 (HDL channel-group strobe-latch aggregation unit) — both ratified 2026-05-26 and both implicit about the channel-group axis. Under PCDN-SOS-09-007 they are now explicitly permitted to diverge via the two-axis surface: D-001 consumes `sos:channel_group` (Rust borrow scope); E-003 consumes `sos:privilege_region` (HDL MPU + access-violation aggregation), defaulting to `sos:channel_group` when unset. The "channel-group" vocabulary is preserved as the umbrella concept; the axis-count question is the only thing PCDN-007 resolves.

**Frozen-enumeration registration policy: Standards Action.** The axis count is a cross-phase contract change — adding a third axis later (e.g. an independent `sos:debug_group` for trace-emit aggregation) requires a §15 amendment to this doc and a ratification session. Demotion to Specification Required would lose the cross-phase invariant that backends only diverge along axes the umbrella ratifies.
...
### 2026-05-26 — PCDN-SOS-09-007 ratification + analyzer/annotation-semantics versioned-pair note

**Status: 🟢 ratified 2026-05-26.**

**Resolution.** PCDN-SOS-09-007 (channel-group as one axis or two) resolved (b) — two axes with default-from-inheritance. The annotation surface gains two new SOS-semantic keys: `sos:channel_group` (Rust borrow scope / `RegisterBlock` boundary, consumed by PCDN-SOS-09-D-001's borrow-enclosure unit) and `sos:privilege_region` (HDL MPU + access-violation aggregation, consumed by PCDN-SOS-09-E-003's strobe-latch aggregation unit). Default rule: when `sos:privilege_region` is omitted, it inherits the value of `sos:channel_group`. The common case is one axis; divergence is a chart-author edit, not a spec migration through both backends. See §15.7 for the full PCDN-007 narrative, options considered, and cross-references to D-001 and E-003.

**Deferred SOS-09-A §5.2 amendment.** The actual annotation-key additions to SOS-09-A's §5.2 ten-key set (expanding it to a twelve-key set adding `sos:channel_group` and `sos:privilege_region`) are NOT included in this commit. Per the Spec-Before-Code discipline, that §15 amendment to SOS-09-A lands as a separate small commit JIT with the SOS-09-D and SOS-09-E implementation PRs that consume the keys. This entry records the obligation; the future SOS-09-A §15 amendment will cite back to this entry as the umbrella authority for the twelve-key expansion. Frozen-enumeration registration policy for the expansion: Standards Action (mirrors SOS-09-A §5.2's existing policy for the ten-key set).

**Analyzer/annotation-semantics versioned-pair discipline.** Future SOS-09 emit backends that mechanize chart-bounds-analyzer outputs into safety-bearing artifacts (PCDN-SOS-09-D-005 codegen-emitted `// SAFETY:` comments being the canonical example as of this entry) MUST treat the chart-bounds analyzer and `docs/concepts/SOS-09-A-CONCEPTS.md` as a versioned pair — they rev together, or the §16 entry on either explicitly names the deliberate skew with the rationale. Backend agents fan out during emit waves; the annotation-semantics doc is single-sourced by one agent per wave to prevent silent vocabulary drift. The discipline applies symmetrically: an analyzer update that changes the discharge surface MUST land with (or co-cite) an SOS-09-A §16 entry; an SOS-09-A §16 entry that changes the annotation vocabulary MUST land with (or co-cite) the analyzer rev that consumes it. Either-side-alone landings reopen the "annotation surface drifts from analyzer semantics" failure mode that PCDN-007 explicitly prevents on the chart-author axis-count surface.

**scjson out-of-scope note.** scjson is NOT in scope for this round. The `other_attributes` JSON preservation surface already covers the future `sos:channel_group` / `sos:privilege_region` keys via the existing `sos:`-prefixed-key convention (per the 2026-05-25 PCDN-SOS-09-001 amendment); no scjson schema amendment is required to admit the two new keys. The scjson round-trip path (INV-SOS-D) is preserved by construction since the new keys ride within the `other_attributes` JSON map iState already round-trips. (See the 2026-05-26 "scjson 0.4.0 feature surface" entry below for the broader roadmap framing of scjson feature uptake; the present round's out-of-scope posture is unchanged.)

**Cross-references.** PCDN-SOS-09-007 narrative in §15.7. PCDN-SOS-09-D-001 (Rust borrow-enclosure unit) and PCDN-SOS-09-E-003 (HDL channel-group strobe-latch aggregation) — both ratified 2026-05-26 by sibling agents in the same wave. Future SOS-09-A §15 amendment (twelve-key expansion) — deferred per the JIT-with-D-and-E-implementation rule above. Test coverage: `tools/sos-codegen/tests/test_sos_09_007_pcdn_ratification.py`.

**Frozen-enumeration registration policy for PCDN-007.** Standards Action — the axis count is a cross-phase contract change. Adding a third axis (e.g. an independent `sos:debug_group` for trace-emit aggregation) requires a §15 amendment to this doc and a ratification session.

### 2026-05-26 — scjson 0.4.0 feature surface: roadmap acknowledgment

**Status: 🟡 informative — roadmap acknowledgment only, NOT a behaviour change to SOS-09 wave-14 scope.**

**Context.** The vendored `scjson` family (parent-repo submodule `ops/packer/submodules/scjson/`, submodule HEAD `74e83da` as of 2026-05-26; Python + JavaScript bindings both at 0.4.0) has landed six features in the 0.3.x → 0.4.0 cycle that materially expand the chart-author preservation surface and the document-composition surface scjson can carry round-trip. This entry records those features so subsequent SOS-09 phases (and any future SOS-08-G / SOS-08-D consumers of the same primitives) have a single in-tree authority to cite when scoping uptake. **These features are AVAILABLE in scjson 0.4.0 but NOT in scope for SOS-09 wave-14.** Integration is roadmap-tracked per `docs/concepts/SOS-ROADMAP-07-PLUS.md` §"scjson 0.4.0 feature integration (roadmap)".

**Features landed in scjson 0.4.0 (umbrella inventory):**

1. **`help_text: list[str]`** — first-class field on 26 SCXML element models per the scjson CONV-E ratification (scjson commits `9008639` / `e1e3d1f`). Distinct from `other_attributes`; carries author-supplied prose. Round-trips through XML ↔ JSON deterministically.
2. **SCXML comment promotion → `help_text`** — XML comments preserved into `help_text` via an 8-rule deterministic attachment algorithm per scjson CONV-F (commits `096f7e3` / `afbb3ac`). Python implementation via `comment_promotion.py` (lxml); JavaScript via fast-xml-parser. Cross-language fixture parity is asserted by the scjson conformance suite.
3. **XInclude support** — `xinclude="preserve"` (default; keeps `<xi:include>` elements in `other_element`) or `xinclude="resolve"` (preprocesses XML to inline the referenced fragments), with an optional `xinclude_base_url` parameter, per scjson CONV-H (commit `5af10c0`).
4. **`<send>` full attribute and sub-element surface** — `src`, `srcexpr`, `target`, `targetexpr`, `type_value`, `typeexpr`, `event`, `eventexpr`, `namelist`, `autoforward`, `delay`, `delayexpr`; plus `content` and `param` sub-elements. Sufficient for the W3C SCXML send vocabulary in full.
5. **`<invoke>` full attribute and sub-element surface** — full attribute set plus `finalize`, `content`, `param` sub-elements.
6. **`other_attributes` registry** — formalized extension-attribute schema per scjson CONV-G (commit `2280480`). Names the contract iState's `other_attributes` extensions (and SOS-09's `sos:`-prefixed-key convention per the 2026-05-25 amendment above) ride within.

**Why these matter to SOS-09 (and adjacent phases) — informative.** Each of the six features sits on the scjson side of the chart-author → AST → backend pipeline that SOS-09 inherits from SOS-06-A / SOS-07. Uptake on the SOS side is non-trivial: it requires (a) walker plumbing inside `tools/sos-codegen/` to thread the new fields through every emit path; (b) template surface changes in each backend (Rust HAL doc-comments, C HAL doc-comments, HDL register-file commentary, membrane-vector annotations); (c) verification coverage across BOTH bindings scjson currently ships (Python + JavaScript) and against the Rust binding that **does not yet exist** in the scjson family; (d) validation-pipeline work to assert round-trip fidelity across all bindings before any chart-author-visible reliance lands. The work is real engineering, not a flip-the-switch ratification.

**Roadmap framing — not in scope for wave-14.** Per the umbrella discipline ("Touching a frozen enum value or an invariant requires a §15 amendment first, in a separate PR"), nothing in this entry alters the SOS-09 frozen vocabulary, the channel-annotation key set, the AuthorityRelationship matrix rows of §8, or the in-scope deliverables of SOS-09-A through SOS-09-G. The six features are **roadmap candidates** for future SOS-09 sub-phases (or, more likely, for `SOS-09-*` and `SOS-08-*` follow-on amendments that mechanize the chart-author doc-comment surface into emitted source / register-map artifacts).

**Cross-references.**
- `docs/concepts/SOS-ROADMAP-07-PLUS.md` §"scjson 0.4.0 feature integration (roadmap)" — the umbrella roadmap entry that breaks the six features down into per-feature scope notes for future ratifications.
- `docs/concepts/SOS-08-G-CONCEPTS.md` §15 (2026-05-26 entry) — XInclude `resolve` mode is named as a tractable compositional mechanism for PCDN-G-wave1-004's nested-chart `chart_path` walking (wave-2 deferred work).
- `docs/concepts/SOS-06-A-EVALUATION.md` — comment-promotion + `help_text` named as future inputs to the not-yet-authored `scjson-gen-c` / `scjson-gen-rust` codegen siblings.

**Frozen-enumeration registration policy.** None — this entry adds no new SOS-side enumeration values, no new annotation keys, no new vocabulary. Any future amendment that mechanizes one of the six scjson features into an SOS-side surface (e.g. emitting `help_text` content as Rust `///` doc-comments in `SOS-09-D`-emitted HAL code) lands as its own normative §15 / §16 amendment in the affected phase doc with its own registration-policy declaration.

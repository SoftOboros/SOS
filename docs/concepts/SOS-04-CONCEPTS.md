...
### 2026-06-03 — Cross-reference: SOS-04-B embeddable-kernel sub-letter ratified (Ira)

[`SOS-04-B-EMBEDDABLE-KERNEL.md`](./SOS-04-B-EMBEDDABLE-KERNEL.md) is **ratified** (2026-06-03). It extends this port from a scheduling/conformance model into an **embeddable RTOS that runs real application task bodies** + a consumable `no_std` kernel-library API — the capability `handlers.rs:242-255` registers as missing ("v1 doesn't run actual tasks"). Ratified: (PCDN-001) extract a `sos-m7-rust-kernel` lib crate; (PCDN-002) task entry ABI `extern "C" fn() -> !` + PSP exception-return-frame priming at `task.create` (Standards Action — SOS-05 mirrors); (PCDN-003) DirectCallBasepri → macrostep → pend-PendSV syscall model. INV-S-EMBED-1 keeps the conformance model/trace byte-equal (the existing 7-vector suite MUST still pass after the lib extraction). No §6.3/§6.4 contract text changes here — SOS-04-B adds a real-execution front-end above the unchanged core. Proposed by the sibling DAA-08 initiative (REQ-SOS-1/2 at real-execution level); DAA-08-B consumes the §5.5 API once it lands.

...
- **FAM-04-A — Make stack sizes configurable.** Per PCDN-006 user-note. Today's hard-coded `KERNEL_STACK_BYTES` / `TASK_STACK_BYTES` become tunable (via a Cargo feature flag, a `build.rs`-generated constants module, or a runtime-init parameter — the amendment chooses). Triggers when a vector exceeds 2 KiB task stack, or when the SOS-06 codegen comparison surfaces stack-size sensitivity, whichever comes first.

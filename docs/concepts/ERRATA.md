...
## ERRATA-011 — `on_sys_tick` macrostep not in the DirectCallBasepri envelope → HSEM give preempts SysTick → KERNEL_STATE corruption → high-prio task wedged

**Status:** 🟢 resolved (2026-06-05).
**First seen:** 2026-06-05 (HEAD `caac79b`, branch `daa08-amp-proposals`), surfaced re-running the DAA-08-A §3 A/B parity once the *real* L1/L2/L3-fixed kernel finally ran on silicon (after ERRATA-010 — every prior `--features sos` binary had linked a stale kernel, so this was never reachable before).
**Owning phase:** SOS-04-B (embeddable kernel — `embed::on_sys_tick` / ISR concurrency discipline).

**Symptom (DAA-08-C bench).** With the I-cache on and the genuinely-fixed kernel, the analyzer-on-SOS audio task (prio 4, the highest) runs correctly for **a few minutes** (~63/s wake + passthrough), then **wedges to 0/s permanently** while the prio-1 render task keeps running (~56/s). No fault (`CFSR/HFSR = 0`). DAA-side bench record: disco-analyzer ERRATA-017 (+ EOQ-011).

**Characterization (bench, datamodel snapshot to SRAM4).** Live `KERNEL_STATE` at the wedge:
- pre-wedge: audio `state = BlkSem`, `sems[0].waiters = [2]`, `count = 0` — correctly blocked-waiting; the give handler demonstrably takes the unblock-waiter branch and pends PendSV with `incoming = 2` (proven by instrumented branch counters).
- at the wedge (min ~7, coincident with a burst of the race counter below): audio flips to `state = Running` while `current = 0/1` (idle/render) — i.e. **`Running` but not `current`, and in no `ready[]`/waiter queue** — and `sems[0].count` saturates at 1. From then on `pick_next` (which only ever re-queues `current`) can never select audio again → permanent starvation.

**Root cause.** `embed::on_sys_tick` ran the `dispatch_event` macrostep + pend **without** the BASEPRI bracket every other real-context entry (`sem_take`/`task_delay`/`sem_give_from_isr` via `run_envelope`) uses. Its comment asserted *"SysTick already runs at `0xC0` (≥ kernel-aware), so no BASEPRI bracket is needed — the ISR context is the mask."* That is **false**: the HSEM `*_from_isr` band is `0xA0` (SOS-00 §6.2), which **outranks** SysTick (`0xC0`), so an HSEM `sem_give_from_isr` **preempts the tick mid-macrostep**. The two handlers then perform interleaved non-atomic read-modify-writes on the shared `current` / `tcb[].state` / `ready[]` (e.g. `pick_next`'s `tcb[n].state = Running` (A) then `current = n` (B) is split by the preemption), leaving the scheduler in the leaked-`Running` state above. A direct **race detector** (a "tick-in-progress" flag the give checks) confirmed the preemption occurs (~3–4×/min; the wedge coincided with a burst). This is the *deeper root* of the same class ERRATA-009 Layer 3 began on: L3 made the **outgoing-slot** write race-free (PendSV-private `LOADED_TID`), but the **whole tick macrostep** was still non-atomic against the `0xA0` band.

**Fix.** Route `on_sys_tick` through the existing `run_envelope` (`embed.rs`) — i.e. `run_envelope(&Event::sys_tick())` — so the tick macrostep runs inside the same DirectCallBasepri envelope (`BASEPRI = 0xA0` → macrostep → unmask → pend PendSV iff `current` changed) the syscall/give paths use. This **defers** any HSEM give until the tick macrostep completes, making it atomic. Pure port-layer concurrency fix; no model/chart/trace change (INV-S-EMBED-1). The faulty "context is the mask" comment is removed.

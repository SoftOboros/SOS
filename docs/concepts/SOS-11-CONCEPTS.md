...
### 2026-05-27 — SOS11AD: PCDN-007 + PCDN-010 ratifications

Combined entry stamping two Wave-5 ratifications from the 2026-05-27 multi-PCDN walkthrough (the same walkthrough that ratified PCDN-008 / PCDN-009 in sibling commits). Filed as one §15 entry by orchestrator request, to avoid an intra-wave change-log conflict between sibling Wave-5 agents touching the bottom of this doc.

**PCDN-SOS-11-007 — state-id collision rewrite policy (option (a) ratified).** 🟢 ratified 2026-05-27. The Wave-3K v1 stance documented under §5.1 / `inline_subchart` ("when a child sub-chart state id collides with any parent peer state id anywhere in the parent chart, the inlined child id is rewritten to `{dispatch_state_id}__{original_child_id}` and every transition target inside the inlined body updates to follow; the dispatch state's own id is preserved verbatim so external parent transitions targeting it survive untouched") is now formally ratified as the v1 normative rewrite scheme. No code change is required — `tools/sos-codegen/sos11_mcp/inline.py` (the Wave-3K landing commit, see the SOS11K1 entry above) is the as-built implementation and this ratification stamps that as-built into normative form. **Registration policy: Specification Required** — the rewriting scheme is a contract-surface decision; future amendments to switch to a different scheme (e.g. monotonic-counter suffix `{child_id}_<n>`, hash-based suffix, or a numbered-rename of the colliding parent id instead) require a §15 amendment + walkthrough naming the replacement scheme. Authority: SOS-11 §5.1 (inline_subchart primitive) and INV-SOS-C (`derive` — collision rewriting is a determinant of the chart-modification surface's referential integrity).

**PCDN-SOS-11-010 — `ValidationAxisReport` `AxisStatus` tristate (option (a) ratified).** 🟢 ratified 2026-05-27. §6's four-axis result contract gains a `status: AxisStatus = AxisStatus.EVALUATED` field on `ValidationAxisReport`, where `AxisStatus` is a new string enum with three values: `EVALUATED` (substrate ran end-to-end; `passed` is substantive), `DEFERRED` (substrate not yet wired into the composer / handler; `passed=True` is non-substantive — the handler isn't claiming failure, it's claiming "we didn't evaluate"), and `NOT_REQUESTED` (caller's `axes=` argument excluded this axis; `passed=True` is non-substantive — the convention preserves the four-tuple shape without committing to a verdict). The `diagnosis` field stays in all three modes — for DEFERRED it cites the not-yet-wired substrate ("deferred: SOS-01-CH-runner not yet wired"); for NOT_REQUESTED it reads "not requested"; for EVALUATED-pass it's `None` or a substrate-specific success diagnosis; for EVALUATED-fail it's the substrate's failure diagnosis. Callers wanting "axes that actually failed substantively" filter `[r for r in report.axis_reports() if r.status == AxisStatus.EVALUATED and not r.passed]` instead of the previous overly-broad `[r for r in report.axis_reports() if not r.passed]`. **Backwards-compatibility.** `passed: bool` stays primary; `status` defaults to `EVALUATED` so existing construction sites that say `ValidationAxisReport(axis=..., passed=True)` continue to behave correctly (silent code paths that pre-date PCDN-010 still claim substantive evaluation, which IS what they meant before the tristate landed). **Registration policy: Standards Action** — adding a fourth status value requires a §15 amendment + walkthrough; the three values mirror SOS-11 §6's three honest-partial reporting modes and adding a fourth would amend the contract surface, not just extend an internal enum.

**Code landed under SOS11AD.**

| File | Change |
|---|---|
| `tools/sos-codegen/sos11_mcp/contracts.py` | New `AxisStatus(str, Enum)` with three values; new `status: AxisStatus = AxisStatus.EVALUATED` field on `ValidationAxisReport`; `to_dict()` carries the `status` field; new `__all__` exports `AxisStatus` |
| `tools/sos-codegen/sos11_mcp/validation.py` | `_resolve` helper now marks unrequested axes as `status=AxisStatus.NOT_REQUESTED`; substantively-evaluated axes default to `EVALUATED` (no explicit change to per-axis runners — they construct `ValidationAxisReport` with the default `status`, which is correct for substantive evaluation including substrate-side failures) |
| `tools/sos-codegen/sos11_mcp/inline.py` | The three deferred axes (lint, bound_converges, invariants_hold) now carry `status=AxisStatus.DEFERRED`; the substantively-evaluated `scjson_round_trip` axis carries explicit `status=AxisStatus.EVALUATED` |
| `tools/sos-codegen/sos11_mcp/extract.py` | The three deferred axes (scjson_round_trip, lint, bound_converges) flip from `passed=False`+`diagnosis="deferred: …"` to `passed=True`+`status=AxisStatus.DEFERRED`+same diagnosis — the prior `passed=False` was over-claiming failure (the substrate hadn't run, so "failed" was an unsubstantiated verdict). The substantively-evaluated `invariants_hold` axis carries explicit `status=AxisStatus.EVALUATED` |
| `tools/sos-codegen/tests/test_sos11_contracts.py` | 3 new tests + 1 updated test (the `all_passed` to_dict expectation gains `"status": "evaluated"` per axis) |
| `tools/sos-codegen/tests/test_sos11_validation.py` | 4 new tests covering the EVALUATED / DEFERRED / NOT_REQUESTED axes status + the substantive-failure filter use-case |
| `tools/sos-codegen/tests/test_sos11_extract.py` | 1 updated test (`test_validation_report_marks_contract_match_passed_others_deferred` now asserts `status == DEFERRED` + `passed is True` for the three not-yet-wired axes) |

**Invariants touched.** PCDN-007 → INV-SOS-C (`mirror`; the §5.1 inline_subchart primitive's collision-rewrite scheme is the chart-modification surface's referential-integrity guarantor — ratifying the as-built scheme codifies the existing modification-path invariant without changing it). PCDN-010 → INV-SOS-C (`derive`; the tristate makes the validation surface — the §7 pre-commit gate — more honestly self-describing about what was and wasn't evaluated) + INV-SOS-H (`derive`; the tristate gives chart-vocabulary diagnoses a precise tag for "we couldn't evaluate" vs "the chart failed", which avoids the false-alarm reporting that the old `passed=False` for deferred axes would cause in any caller iterating `failed_axes()`). No invariant relationships changed; no §13 row required restating.

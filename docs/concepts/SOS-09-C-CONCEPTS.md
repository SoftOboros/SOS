...
- (c) ✅ The `tools/sos-codegen/c_hal_emit.py` emission walker exists and produces at least one chart channel's worth of C HAL header content covering: a `#define`-form register address; a `volatile`-qualified struct overlay typedef; at least one accessor of each emitted `<op>` shape applicable to the channel's kind (per §5.1); at least one `static const unsigned` field-shift constant. Implemented + tested in `tools/sos-codegen/tests/test_c_hal_emit.py::test_minimal_emission_carries_define_addr_struct_accessor_field_const`.
- (d) ✅ Accessor naming follows the `SOS_C_<channel>_<op>` convention deterministically from `sos:name` (per INV-S-MEM-C-5). Verified in `test_accessor_naming_follows_convention` + `test_each_kind_emits_its_full_op_set`.
- (e) ✅ `volatile` qualifier appears on every struct-overlay field (per §5.2 / INV-S-MEM-C-2 indirect). Verified in `test_struct_overlay_volatile_on_every_field`.
- (f) ✅ Side-effect-bearing accessors use `*_consume_*` / `*_fire_*` discriminating names per §5.3 (NOT plain `read` / `write`); a chart channel with `sos:clear_on_read="true"` emits `consume` and NOT `read` (per INV-S-MEM-C-2). Verified in `test_clear_on_read_emits_consume_not_read` + `test_command_emits_fire_not_write`.
- (g) ✅ The emitted headers pass `gcc -Wall -Wextra -Wpedantic -std=c11` AND `clang -Wall -Wextra -Wpedantic -std=c11` with exit 0 (per INV-S-MEM-C-4). Verified in `test_emitted_headers_compile_clean_wpedantic_c11` (single-compiler — runs when either gcc or clang is on PATH) and `test_emitted_headers_compile_clean_under_both_compilers` (cross-compiler — runs only when both are on PATH). Both gates skip cleanly if no compiler is available (e.g. minimal CI image).
- (h) ✅ For every chart channel with `sos:zone` annotation, an `extern const sos_mpu_region_t sos_mpu_<channel>_region;` declaration is emitted (per §5.6). Verified in `test_zone_annotated_channel_emits_mpu_extern` + `test_umbrella_includes_sos_mpu_h`.
- (i) ✅ Deterministic-from-`sos:name`: two emit runs against charts differing only in `sos:id` UUID values produce byte-identical accessor-symbol output (per INV-S-MEM-C-5). Verified in `test_byte_identical_when_only_sos_id_differs` + `test_accessor_symbols_carry_no_uuid_substring`.
- (j) ✅ Cross-phase invariants INV-SOS-A through H cited correctly in the SOS-09-C emit-path source; cross-sub-phase invariants INV-S-MEM-1 through 6 cited correctly per sub-phase; INV-S-MEM-C-1 through 5 declared in this doc's §6. Source-side citation check enforced by `test_emitter_source_cites_cross_phase_anchors`.
...
### 2026-05-26 — SOS09C1 implementation (Ira)

Implementation phase landed for SOS-09-C. Acceptance gates (c)–(j) flipped from ⏸ to ✅ in §12 of this doc. Summary of what landed:

- **Emitter module**: `tools/sos-codegen/c_hal_emit.py` (new sibling of `transliterate_c.py` per §10 reconciliation; file-disjoint from the existing ECMAScript→C transliterator). Public surface: `emit_c_hal(annotations, *, chart_name, base_address=0x40000000, output_dir=None) -> dict[str, str]` and `emit_c_hal_from_chart(chart_path, **kwargs) -> dict[str, str]`. Emits the umbrella `sos_<chart>.h` header transitively including every per-group `sos_<chart>__<group>.h` sub-header (per PCDN-SOS-09-C-004 (a)); each sub-header carries `#define <chan>_ADDR` constants, the `volatile`-qualified `sos_<chart>__<group>_regs_t` struct overlay (per PCDN-SOS-09-C-002 (a)), `static const unsigned <chan>_<field>_SHIFT/MASK` constants (per PCDN-SOS-09-C-001 (b) + §5.5), `static inline` accessor functions (per PCDN-SOS-09-C-005 (a)) with side-effect-discriminating names (`_consume` / `_fire` / `_claim` / `_release` per §5.3), and `extern const sos_mpu_region_t sos_mpu_<channel>_region;` declarations for every channel with a `sos:zone` annotation (per §5.6).
- **Determinism (INV-S-MEM-C-5)**: emitted accessor symbol names derive from `sos:name` only. The chart's `sos:id` UUIDs do NOT participate in any emitted symbol; two charts that differ only in `sos:id` values produce byte-identical output. Group ordering is preserved from the SOS-09-A walker's document-order channel list (dict insertion order).
- **Cumulative address layout**: matches `transliterate_svd._channel_size_bytes` so the SVD and the C HAL agree on every per-channel `addressOffset` (per §10 vs SOS-09-B reconciliation). Narrow channels (`sos:width < 32`) are padded in the struct overlay so the cumulative offset chain matches SVD's 4-byte minimum slot.
- **Tests**: `tools/sos-codegen/tests/test_c_hal_emit.py` (25 tests). Covers one channel per kind (`status`/`command`/`queue`/`shared`), clear-on-read → `_consume` discrimination, command → `_fire` discrimination, MPU extern emission, per-field shift/mask constants, deterministic-from-`sos:name`, byte-identical determinism under `sos:id`-only differences, header-guard sanity, output-dir disk write, source-side cross-phase-anchor citation. Toolchain cleanness gate (g) compiles the emitted umbrella header with `-Wall -Wextra -Wpedantic -Werror -std=c11` under any available `gcc` / `clang` (single-compiler gate runs whenever either is on PATH; cross-compiler gate runs only when both are present; both skip cleanly when no C compiler is available).
- **Fixture**: `tools/sos-codegen/tests/fixtures/sos_09_c/sos09_c_hal_chart.scxml` (six channels covering all four kinds, mirroring the SOS-09-B fixture shape so the two emit paths can cross-validate layout determinism).

No spec amendments triggered — the ratified `-CONCEPTS.md` text already encoded the implemented behaviour. §12 gate (c)–(j) status flipped to ✅. All 2056 tests pass under `RUSTFLAGS="" python -m pytest tools/sos-codegen/tests/ -x -q` (baseline 2031 + 25 new SOS-09-C tests).

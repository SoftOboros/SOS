...
- **PCDN-G-wave1-004 — Nested-chart `chart_path` walking → DEFERRED to wave-2**. Wave-1 emits `chart_path=[chart_state]` (single-segment, top-level chart only). Wave-2 walks the SCXML hierarchy at emit time using `ChartAst.raw_scjson` (the field added to `ChartAst` in SOS-08-C wave-2) so sub-chart transitions carry the full `/parent/child/grandchild` path per [SOS-12][sos-12] recursive-dispatch vocabulary, capped at depth 8 per PCDN-G-002. INV-S-HDL-G-2 (chart-vocabulary mandatory) currently upheld by the single-segment form; nested-chart compositions ratifying §10's [SOS-12][sos-12] reconciliation need the walk. **Note (added 2026-05-26):** scjson 0.4.0's XInclude support (`xinclude="resolve"` mode, scjson CONV-H commit `5af10c0`) becomes a tractable compositional mechanism for the multi-segment hierarchical walk: a `<state>` whose body is sourced from an `<xi:include>` reference can be pre-resolved to its inline form before the wave-2 walker reads `ChartAst.raw_scjson`, so cross-file sub-chart factoring stays transparent to the path-building loop. The wave-2 walker MAY consume `xinclude="resolve"`-preprocessed `ChartAst.raw_scjson` as one input shape; the existing in-file nested-state shape remains the primary case. No wave-1 surface change; this is a wave-2 input-shape expansion. See `docs/concepts/SOS-09-CONCEPTS.md` §16 (2026-05-26 "scjson 0.4.0 feature surface" entry) and `docs/concepts/SOS-ROADMAP-07-PLUS.md` §"scjson 0.4.0 feature integration (roadmap)" for the umbrella roadmap framing.
...
### 2026-05-26 — PCDN-G-wave1-004 wave-2 input-shape note (scjson 0.4.0 XInclude)

**Status: 🟡 informative — roadmap acknowledgment for the still-deferred wave-2 walker work; NOT a behaviour change to wave-1 or wave-3c.**

**Context.** The vendored `scjson` family (parent-repo submodule `ops/packer/submodules/scjson/`, submodule HEAD `74e83da` as of 2026-05-26; Python + JavaScript bindings both at 0.4.0) landed XInclude support in the 0.3.x → 0.4.0 cycle per scjson CONV-H (commit `5af10c0`). The feature exposes two modes: `xinclude="preserve"` (default; `<xi:include>` elements are retained in `other_element` so the AST round-trips losslessly) and `xinclude="resolve"` (the parser preprocesses the XML to inline referenced fragments before AST construction). An optional `xinclude_base_url` parameter lets the resolver resolve relative paths against a non-default base.

**Why this matters for PCDN-G-wave1-004.** The wave-2 nested-`chart_path` walker (still deferred per §15 PCDN-walkthrough entry above) walks `ChartAst.raw_scjson` to build the `/parent/child/grandchild` path. With XInclude resolution available upstream of the walker, a chart-author factoring of sub-charts across files via `<xi:include>` becomes transparent to the wave-2 walker — the resolved AST presents the same shape as an inlined sub-chart, so the path-building loop does not need an `xi:include`-aware branch. This is a tractable compositional mechanism for the multi-segment hierarchical walk; without it, cross-file sub-chart factoring would need an SOS-side resolver path or a chart-author restriction to single-file charts.

**Wave-2 scope clarification.** The wave-2 walker MAY consume `xinclude="resolve"`-preprocessed `ChartAst.raw_scjson` as one input shape. The primary input shape remains in-file nested-state (the wave-1 chart `rtos_kernel.scxml` shape). No wave-1 emit surface changes. No wave-3a / 3b / 3c emit surface changes. The XInclude support is opt-in upstream — chart authors who do not use `<xi:include>` see no behavioural difference; chart authors who do see the `chart_path` walk work transparently across the file boundary.

**Roadmap framing — wave-2 work remains deferred.** This entry does not unblock or reschedule PCDN-G-wave1-004; it only names XInclude as one of the input-shape considerations the wave-2 walker will face. The walker's actual landing is still future work, gated on (a) the SOS-08-C `ChartAst.raw_scjson` field shape (already landed in SOS-08-C wave-2), and (b) verification that the walker handles arbitrary nesting depth up to PCDN-G-002's depth-8 cap per [SOS-12][sos-12].

**Cross-references.**
- `docs/concepts/SOS-09-CONCEPTS.md` §16 (2026-05-26 "scjson 0.4.0 feature surface" entry) — umbrella roadmap acknowledgment.
- `docs/concepts/SOS-ROADMAP-07-PLUS.md` §"scjson 0.4.0 feature integration (roadmap)" — per-feature scope notes, including XInclude.
- scjson CONV-H (commit `5af10c0` in `ops/packer/submodules/scjson/`).

**Invariants.** INV-S-HDL-G-1 through G-6 unchanged. INV-SOS-A (chart-as-source) preserved — XInclude-resolution operates on the SCXML on-disk artifact upstream of the walker; the chart-author authoring surface (iState) is unaffected. INV-SOS-D (iState authoring, SCXML canonical) preserved by construction since `xinclude="preserve"` (the default) keeps `<xi:include>` elements in `other_element`, so an iState-extracted chart round-trips losslessly without any XInclude work at all.

**Frozen-enumeration registration policy.** None — this entry adds no new SOS-08-G enumeration values, no new annotation fields, no new conformance gates. Any future amendment that mechanizes XInclude into an SOS-08-G surface (e.g. the wave-2 walker explicitly supporting it) lands as its own §15 entry with its own registration-policy declaration.

**Honest framing.** Wave-2 is not on a specific schedule; this entry exists to record the input-shape acknowledgment so the wave-2 PR has a citation to point at. The verification work (round-trip fidelity across resolved-vs-preserved modes) lives upstream in the scjson project; SOS-08-G's wave-2 walker consumes whichever shape the chart presents.

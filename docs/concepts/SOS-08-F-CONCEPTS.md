...
- **`transliterate_hdl_uvm_seq.py` walker** (~590 LOC) emits three artifacts per chart under `uvm/<chart>/`:
    - `sos_uvm_seq_pkg.sv` — consolidated SystemVerilog package per §5.7 + PCDN-SOS-08-F-006. Contains the `sos_event_family_e` enum (six tags per §6.1 + PCDN-SOS-08-F-002), the `sos_chart_event_s` struct (vector-IR row shape per §6.4), the baseline `sos_seq_item` class (universal discriminated union per §5.3 + PCDN-SOS-08-F-003 with all three chart-vocabulary metadata fields per §6.2 + INV-S-HDL-F-3), and six per-family `uvm_sequence` subclasses (`sos_task_sequence`, `sos_sem_sequence`, `sos_queue_sequence`, `sos_timer_sequence`, `sos_event_sequence`, `sos_tick_sequence`) following the §6.3 template with `pre_body` / `post_body` empty virtual hooks per §5.6 + PCDN-SOS-08-F-005.
    - `sos_uvm_seq_pkg.svh` — header file with typedef + enum exports for split-compilation drivers. Customer's driver can `\`include` it without importing the full sequence library.
    - `sos_uvm_integration_example.sv` — informative worked example per §6.5. Demonstrates the five-step customer-integration contract (package import; sequencer typedef; driver implementation; sequence start; failure handling with §6.6 chart-vocabulary message format). Marked INFORMATIVE in the file header — not part of the customer's normative test scaffolding.
- **Family enumeration frozen at six** (PCDN-SOS-08-F-002 resolution): `CHART_EVENT_FAMILIES = ("task", "sem", "queue", "timer", "event", "tick")`. Adding a seventh family requires a §15 amendment.
- **Universal `sos_seq_item` shape** (PCDN-SOS-08-F-003 resolution): one transaction class with `family : sos_event_family_e` discriminator + `event_id` / `payload_data` / `chart_state` / `transition_id` / `invariant_id` fields. The customer's driver branches on `family`; per-family transaction classes were rejected to avoid multiplying the customer's driver-registration touch points.
- **In-package factory registration** (PCDN-SOS-08-F-004 resolution): `\`uvm_object_utils*` macros emit alongside each class in the package; customer does NOT register SOS classes manually. Standard UVM `set_type_override_by_type` mechanics remain available for customer override.
- **UVM 1.2 grammar discipline** (PCDN-SOS-08-F-001 resolution): walker uses only `uvm_sequence`, `uvm_sequence_item`, `uvm_object_utils`, `uvm_field_*`, `uvm_pkg`, `uvm_macros`. Forward-compatibility with UVM 2.0 (IEEE 1800.2-2020) verified by test (`TestInvariants.test_inv_f5_uvm_1_2_grammar_only` enumerates the allowed UVM token set; any new `uvm_*` reference fails the test pending §15 amendment confirming cross-version compatibility).
- **Invariant audit by construction** (`_audit_all`): every emitted file is post-pass scanned for INV-S-HDL-F-1 (no `extends uvm_env|uvm_agent|uvm_sequencer|uvm_driver|uvm_monitor|uvm_scoreboard|uvm_test`, no `uvm_config_db` references), INV-S-HDL-F-3 (`sos_seq_item` carries `chart_state` / `transition_id` / `invariant_id`), and INV-S-HDL-F-4 (no `assert property`, no `bind` directive). Any hit raises `InvariantAuditError` — signals an internal walker bug. The integration example file is exempted from INV-S-HDL-F-1 because it deliberately authors a `uvm_driver` demo shape to illustrate the customer-side contract; the file's INFORMATIVE marker in the header is the formal exemption pinning.
- **CLI integration** in `main.py`: new `--target sos-08-f` choice + `_render_sos_08_f_target` dispatcher (lazy-imports `transliterate_hdl_uvm_seq` mirroring the SOS-08-D / SOS-08-E pattern). Reuses the existing `cocotb_sva_config` dict (already carrying `chart_name`) for the walker's config arg. The dry-run + `--out` directory-write paths handle dict-output via the same code path as SOS-08-D / SOS-08-E.
- **End-to-end tests** at `tools/sos-codegen/tests/test_transliterate_hdl_uvm_seq.py` (78 tests, all passing). Coverage: file-set; package structure; baseline `sos_seq_item` field-set and `\`uvm_object_utils_*` macros; six per-family sequence subclasses (parameterized over the family list); discriminated-union enum; header file (include guard + exports); integration example five-step contract; invariant satisfaction audit (F-1, F-3, F-4, F-5 grammar gate); audit error paths; input validation; parallel-chart acceptance (chart-agnostic).

**Wave-1 scope (what landed vs. what is deferred)**:

- The per-family sequence's `body()` task calls `load_vector_ir(path, events)` to populate the chart-event queue. **Wave-1 ships `load_vector_ir` as a stub task** that returns an empty queue — the customer's test wires it to their chart vector source until wave-2 lands the full SOS-03 JSONL parser (co-deferred with SOS-08-E's full vector-schema consumer). This is the realistic v1 landing per §12 second-tier conformance ("reduced conformance without (e) end-to-end example"): gates (a)-(d) + (g)-(i) satisfied; gate (e) end-to-end customer-side run requires the customer to provide the env around the SOS sequences.
- Acceptance gate (j) — sibling SOS-08-D + SOS-08-E §10 reconciliation cross-references — is satisfied by their existing §10 sections naming SOS-08-F as the UVM-target sibling. No edits to sibling docs required.
- The integration example uses `sos_sem_sequence` as the worked instance; customers adapt the shape to whichever family their first chart-event demands. The shape generalises trivially across the six families.

**Wave-2 candidates** (recorded explicitly so the boundary is unambiguous):

- **Full SOS-03 vector-IR JSONL parser**: replace the `load_vector_ir` stub with a real parser that reads the chart's `vectors/<family>_chart_bound.jsonl` file and populates the `sos_chart_event_s` queue. Co-deferred with SOS-08-E's full vector-schema consumer so the parser is authored once and reused by both UVM sequences and SV testbench drivers.
- **Acceptance gate (e) end-to-end worked example**: a complete UVM 1.2 env (customer-side scaffolding — sequencer, agent, driver, scoreboard, test) running a SOS-emitted sequence library against a synthetic DUT. Validates the §6.5 five-step integration contract on a reference UVM-supporting flow (Verilator + uvm-python sequence-runner, or a vendor evaluation simulator). Wave-1 satisfies the reduced second-tier conformance without this gate; full first-tier conformance is wave-2.
- **Acceptance gate (f) injected-violation chart-vocabulary check**: manual mutation of a chart vector in the wave-2 worked example produces a UVM error with `chart_state` / `transition_id` / `invariant_id` present in the message. Verifies §6.6 + INV-S-HDL-F-3 against a live UVM env (not just walker-side audit).
- **UVM 2.0 cross-runtime smoke test**: per the INV-S-HDL-F-5 wording ("a per-release smoke run validates the cross-runtime invariant"), wave-2 adds a CI job that compiles + elaborates `sos_uvm_seq_pkg.sv` against both UVM 1.2 and UVM 2.0 reference releases. Wave-1 audits the grammar by allowed-token-list; wave-2 audits by actual cross-runtime compilation.
- **pyuvm overlay** (per umbrella PCDN-004 follow-on): potential SOS-08-F-A sub-phase emitting Python uvm-compatible sequences for the cocotb path. Wave-2 ratification needed if a customer asks for it; wave-1 ships pure SystemVerilog UVM only.

**Cited invariants** (all upheld by wave-1 surface): INV-S-HDL-F-1 (sequences only — audit passes; integration example is INFORMATIVE-exempted by design), INV-S-HDL-F-2 (customer owns env/scoreboard/driver/factory — wave-1 emits nothing outside the sequence library), INV-S-HDL-F-3 (chart-vocabulary fields on `sos_seq_item` — audit verifies presence; per-family `body()` populates from `sos_chart_event_s` rows), INV-S-HDL-F-4 (no SVA emission — audit passes; sibling SOS-08-D + SOS-08-E own the SVA bind files), INV-S-HDL-F-5 (UVM 1.2 grammar only — `test_inv_f5_uvm_1_2_grammar_only` enumerates the allowed `uvm_*` token set).

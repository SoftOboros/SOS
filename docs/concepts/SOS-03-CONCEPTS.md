...
### 2026-05-27 — SOS03-09/12 vector-framework extensions

This amendment closes two outstanding co-land obligations on `SOS-03-CONCEPTS.md` in a single combined entry:

- **Part A** closes [SOS-09 umbrella §12 gate (g)](./SOS-09-CONCEPTS.md#12-acceptance-checklist) — "SOS-03 §15 amendment co-landed extending the vector framework to the six membrane-vector shapes" — by absorbing the SOS-09-F (Membrane vectors) family enumeration into the SOS-03 §5.1 `VectorCategory` surface. See also [SOS-09 §10 vs. SOS-03](./SOS-09-CONCEPTS.md#vs-sos-03-vector-framework) for the umbrella's expectation of this co-amendment.
- **Part B** closes the SOS-12 boundary-vector subtype-absorption gap named by the [2026-05-27 SOS12C1 §15 entry](./SOS-12-CONCEPTS.md#change-log) on `SOS-12-CONCEPTS.md` (boundary-vector emitter landed at `tools/sos-codegen/sos12_boundary_vectors.py`; the per-record `category: "Boundary"` written by the emitter required a corresponding extension to this doc's `VectorCategory` enum, otherwise the SOS-03 schema check rejects the emitted records).

The amendment is **purely additive** — no prior §15 amendment is superseded; the existing five-value `VectorCategory` enum (`Smoke` / `Boundary`[note] / `Stress` / `Regression` / `Diversity`) grows two new categories. [Note: the legacy `Boundary` value at §5.1 refers to *edge-case-of-a-primitive's-contract* vectors, NOT to SOS-12 dispatch-boundary vectors; the names collide in English but the two surfaces are distinguished by category-value + subtype-field per Part B below. The SOS-12 dispatch-boundary value reuses the literal string `"Boundary"` because the SOS-12 §7.2 emitter writes `"category": "Boundary"` verbatim per its 2026-05-27 ratification; the on-disk subdirectories disambiguate (`boundary/` is the legacy edge-case surface; SOS-12 boundary vectors live under `boundary/sos12/` per the directory-disambiguation rule below).]

Both new categories follow the **category-plus-subtype** scheme — the top-level `VectorCategory` value names the surface (`Membrane`, `Boundary`); a required `subtype` field on the vector record names the per-surface shape (one of the six membrane-vector subtypes; one of the four SOS-12 boundary-vector subtypes). The scheme is the lowest-impact extension consistent with §5/§6 existing precedent: it preserves the flat enum at the directory-naming layer (`smoke/`, `boundary/`, `stress/`, `regression/`, `diversity/`, `membrane/`, plus `boundary/sos12/` as a name-disambiguated subdirectory for the dispatch-boundary surface) while adding the subtype axis that §5.1's existing legacy values lack.

**Per INV-SOS-B** (vectors-as-deliverable-at-every-layer, [SOS-07 §6](./SOS-07-CONCEPTS.md)): both new categories operate at their respective layer — `Membrane` vectors at the HW/SW boundary per [SOS-09-F](./SOS-09-F-CONCEPTS.md), `Boundary` vectors at the dispatch boundary per [SOS-12 §7.2](./SOS-12-CONCEPTS.md). Both categories' emitted records carry the chart-vocabulary traceability metadata INV-SOS-H mandates (the membrane vector's `MV-<UUID>-<family>-<seq>` `trace_key` per SOS-09-F §5.3; the boundary vector's `chart_path` / `originating_invariant` block per SOS-12 §7.2).

#### Part A — `Membrane` category absorption (closes SOS-09 umbrella §12 (g))

The §5.1 `VectorCategory` enum is extended with the new value `Membrane`; the §5.4 `VectorOrigin` enum gains no new value (membrane vectors carry `origin: "authored"` at v1; a future codegen-mined origin tracks under `VectorOrigin`'s registration policy when ratified).

**The six membrane-vector subtypes** — sourced verbatim from [SOS-09-F §5.1](./SOS-09-F-CONCEPTS.md#51-vector-family-enumeration) and cross-referenced to [SOS-09 §6 SOS-09-F](./SOS-09-CONCEPTS.md#sos-09-f---membrane-vectors) item-list (which numbers them 1-6 in narrative order):

| `subtype` value | Source (SOS-09 §6 narrative ↔ SOS-09-F §5.1 enum) | One-line shape |
|---|---|---|
| `initial_value` | SOS-09 §6 SOS-09-F item 1 "Initial-value read" ↔ SOS-09-F §5.1 `initial_value` | Read the chart-declared register; assert the chart-declared reset value (the **status-channel read-pair**). |
| `write_then_read` | SOS-09 §6 SOS-09-F item 2 "Write-then-read" ↔ SOS-09-F §5.1 `write_then_read` | Write a value (constrained to field-width and chart-declared legal-value set); read; assert the value matches modulo declared side effects (the **command-channel write-handshake** + the **queue-channel enqueue-dequeue** pairing). |
| `side_effect` | SOS-09 §6 SOS-09-F item 3 "Side-effect-on-write" ↔ SOS-09-F §5.1 `side_effect` | Write a value; assert the chart-declared HW-side action fired (visible via a status register, an IRQ count, or a queue-depth change) — the **side-effect-on-write notification** subtype. |
| `clear_on_read` | SOS-09 §6 SOS-09-F item 4 "Clear-on-read" ↔ SOS-09-F §5.1 `clear_on_read` | Read; assert chart-declared bits cleared; assert second read returns the cleared value — the **clear-on-read semantics** subtype. |
| `atomicity` | SOS-09 §6 SOS-09-F item 5 "Atomicity" ↔ SOS-09-F §5.1 `atomicity` | Under chart-declared concurrent producers (HW and SW writing simultaneously where the chart permits), assert no torn read; for `mutex-required` registers, assert mutex serialises access — the **shared-channel atomic-read-modify-write** subtype. |
| `protection` | SOS-09 §6 SOS-09-F item 6 "Protection" ↔ SOS-09-F §5.1 `protection` | Write from a non-permitted zone; assert the access is rejected and the SW side sees the chart-declared exception path fire. |

The six "membrane-vector shapes derived from the four channel kinds × the read/write pairings" framing maps to the SOS-09-F §5.1 applicability table verbatim (the four `kind` values `status` / `command` / `queue` / `shared` each emit a kind-specific subset of the six families; not every kind emits every family — see SOS-09-F §5.1 for the kind→family applicability matrix).

**Per-vector required metadata** (additional to the SOS-03 §6 base fields `name` / `description` / `category` / `origin` / `tags` / `config` / `input` / `expected_trace`). Per [SOS-09-A §5.2 twelve-key set](./SOS-09-A-CONCEPTS.md#52-permitted-sos-prefixed-key-set), every `Membrane`-category vector MUST carry:

- `channel_id` — the channel's `sos:id` UUID (RFC 4122 canonical hyphenated form, per SOS-09-A §5.2 row 1). Identity-only handle; rename-stable per SOS-09-F INV-S-MEM-F-3.
- `kind` — one of the four channel kinds per SOS-09 §5.1 (`status` / `command` / `queue` / `shared`); mirrored from SOS-09-A §5.2 row 3.
- `dir` — channel direction per SOS-09 §5.2 (`hw→sw` / `sw→hw` / `bidirectional`); mirrored from SOS-09-A §5.2 row 4.
- `register_offset` — the chart-declared register offset within the channel-group's address space (sourced via SOS-09-B's CMSIS-SVD emission; the offset is the canonical numeric address SOS-09 emitters compute from the chart annotation set).
- `protection_zone` — one of `privileged` / `unprivileged` per SOS-09 §5.4 / SOS-09-A §5.2 row 5; default `privileged`.

The `subtype` field is required for every `Membrane`-category vector; the value MUST be one of the six listed above.

**Co-simulation harness (v1 execution substrate for membrane vectors).** Per [SOS-09 PCDN-005 ratification 2026-05-23](./SOS-09-CONCEPTS.md#change-log) (re-ratified through [SOS-09-F §5.4](./SOS-09-F-CONCEPTS.md#54-co-simulation-harness)), the v1 execution substrate for `Membrane`-category vectors is **cocotb with a Python CPU stub** — the cocotb test instantiates the SOS-09-E-emitted HDL register-file RTL and drives the SW side as a Python stub model of register reads/writes per the SOS-09-F §5.4 `harness.bus.{read,write}` / `harness.wait_irq` / `harness.concurrent_writer` / `harness.{install_mpu,set_zone,seed}` primitive set. Cycle-accurate ISS integration and real-hardware-loopback adapters are deferred per SOS-09-F §5.4. This substrate is NOT the SOS-03 §7.6 port-binary contract; `Membrane` vectors do NOT round-trip through the §7.6 stdin/stdout port binary, because the §6 base `input` / `expected_trace` shape does not capture HW-side stimulus or HW-side observation. Future SOS-03 §15 amendments MAY ratify a SOS-09-F→SOS-03 round-trip adapter; v1 keeps the surfaces separate.

**Directory placement.** `conformance/vectors/membrane/<channel_name>/<NNNN>-<subtype>.json` (the `<channel_name>` is the SOS-09-A `sos:name` SV-identifier; the channel UUID lives inside the file's `channel_id` field, not in the path, to keep file paths human-readable per SOS-03 §6.3 rationale). The implementation commit that lands the SOS-09-F walker output (under `tools/sos-codegen/`) is the one that creates the `conformance/vectors/membrane/` subtree; the SOS-03 §6.1 directory-layout text is correspondingly extended at the next ratification round if naming drift emerges. The harness's default scan picks up the new subtree without code change (`**/*.json` glob per §7.5).

**Frozen-enumeration registration policy for the six membrane-vector subtype set: Standards Action.** Adding a seventh membrane-vector subtype requires a §15 amendment on THIS doc AND a co-citing §16 amendment on `SOS-09-F-CONCEPTS.md` §5.1. This mirrors the SOS-09-F §5.1 frozen-enumeration policy verbatim — the subtype enum is the cross-phase contract surface; both phase docs hold mutation rights jointly.

#### Part B — `Boundary` category absorption (closes the gap Wave-1C named)

The §5.1 `VectorCategory` enum is extended with the new value `Boundary` (sibling to `Membrane`). Per the legacy-name disambiguation note at the top of this amendment, on-disk SOS-12 dispatch-boundary vectors live under `conformance/vectors/boundary/sos12/<parent_chart_id>/<child_chart_id>/<NNNN>-<subtype>.json` to keep the directory tree distinguishable from the legacy `boundary/` edge-case-of-a-primitive surface. The category enum value is the same string (`"Boundary"`) for both surfaces because the SOS-12 §7.2 emitter at [`tools/sos-codegen/sos12_boundary_vectors.py`](../../tools/sos-codegen/sos12_boundary_vectors.py) writes `"category": "Boundary"` verbatim per its 2026-05-27 ratification; coordinated renaming of either surface is a future §15 amendment if confusion emerges.

**Boundary vector metadata block** — the four fields the SOS-12 §7.2 emitter ratified at 2026-05-27 (sourced verbatim from [the boundary-vector emitter at `tools/sos-codegen/sos12_boundary_vectors.py`](../../tools/sos-codegen/sos12_boundary_vectors.py) and [SOS-12 §7.2](./SOS-12-CONCEPTS.md#72-boundary-vectors)). Every `Boundary`-category vector record MUST carry a top-level `metadata` block with:

- `chart_path` — the dispatch-tree address the vector probes, of shape `<parent_chart_id>.<dispatch_state>.<child_chart_id>` per the emitter's `_chart_path()` helper.
- `trigger` — the event name (or `dispatch.round_trip` for invariant subtypes) that drives the vector at the dispatch edge.
- `expected` — the chart-vocabulary expected observation (a state-id, an event-name, or the literal `invariant_holds`).
- `originating_invariant` — the chart's invariant id for the `invariants_maintained` / `invariants_assumed` subtypes; `null` for the `events_in` / `events_out` subtypes.

The metadata block satisfies INV-SOS-H (vector-to-chart traceability) for the dispatch-boundary surface: a failing boundary vector renders as "at `<chart_path>`, trigger `<trigger>` produced unexpected `<actual>` — expected `<expected>`; originating invariant `<originating_invariant>`" — chart vocabulary throughout.

**The four boundary-vector subtypes** — sourced verbatim from [SOS-12 §7.2](./SOS-12-CONCEPTS.md#72-boundary-vectors) (which enumerates them as four bullet points: events-in / events-out / datamodel-boundary-fields / invariants-assumed-of-environment) and from the emitter's `kind`-field values. Per the umbrella enumeration `{events_in, events_out, invariants_maintained, invariants_assumed}`:

| `subtype` value | Source (SOS-12 §7.2 ↔ emitter `kind` field) | One-line shape |
|---|---|---|
| `events_in` | §7.2 bullet 1 ↔ emitter `kind: "events_in"` | Every event in the contract's `events_in` produces the parent-observable behaviour the contract declares (parent sends event; sub-chart, treated as opaque, behaves as if its contract is the spec). |
| `events_out` | §7.2 bullet 2 ↔ emitter `kind: "events_out"` | Every event in the contract's `events_out` is observed by the parent in the contract-declared sequence. |
| `invariants_maintained` | §7.2 (contract field `invariants_maintained` per [SOS-12 §5.1 contract shape](./SOS-12-CONCEPTS.md#51-sub-chart-contract-shape)) ↔ emitter per-vector `kind: "invariant_maintained"` (singular) | The sub-chart preserves each `invariants_maintained` invariant across the dispatch round-trip. |
| `invariants_assumed` | §7.2 (contract field `invariants_assumed` per §5.1; `invariants.assumed_of_environment`) ↔ emitter per-vector `kind: "invariant_assumed"` (singular) | The parent discharges each `invariants_assumed` environmental invariant at the dispatch instant. |

**Implementation note on plural-vs-singular.** The contract-field names (per [SOS-12 §5.1 contract shape](./SOS-12-CONCEPTS.md#51-sub-chart-contract-shape) and the emitter's `SubChartContract` dataclass fields at lines 127-130 of `tools/sos-codegen/sos12_boundary_vectors.py`) are plural — `invariants_maintained` and `invariants_assumed`. The per-vector `kind` field values emitted by the walker are singular — `invariant_maintained` and `invariant_assumed` — because each emitted record covers exactly one invariant. Both shapes are normative: the plural form names the subtype as a class-of-vectors; the singular form is the per-record `kind` discriminator. The four subtype names in the table above are the class-form; the as-emitted per-record `kind` field uses the singular form for the two invariant rows. Future SOS-03 / SOS-12 amendments MAY collapse to one shape if downstream confusion emerges; v1 preserves the as-built emitter behaviour.

**Note on the §7.2 third clause `datamodel_boundary` reads/writes.** SOS-12 §7.2 names a third generator clause ("the `datamodel_boundary` reads/writes happen in the contract-declared order"); the 2026-05-27 SOS12C1 emitter does not emit a separate `datamodel_boundary` subtype at v1 — datamodel-boundary ordering is verified inline within the `events_in` / `events_out` vector sequences. A future amendment ratifying a separate `datamodel_boundary` subtype tracks under this enum's Standards-Action policy.

**Vector ID shape.** Per the SOS12C1 emitter's `_vector_id()` helper (`tools/sos-codegen/sos12_boundary_vectors.py:263`), the ratified shape is `sos12-boundary-<parent>-<child>-NNNN` (4-digit zero-padded sequential per `(parent, child)` edge). The shape mirrors the SOS-09-F `MV-<UUID>-<family>-<seq>` traceability convention. The `vector_id` field is required for every `Boundary`-category vector.

**Frozen-enumeration registration policy for the four boundary-vector subtype set: Standards Action.** The four subtypes are the SOS-12 §5.1 / §7.2 contract surface; adding a fifth (e.g. promoting `datamodel_boundary` to its own subtype) requires a §15 amendment on THIS doc AND a co-citing §15 amendment on `SOS-12-CONCEPTS.md` §7.2. This mirrors SOS-12 §5.1's frozen-enumeration policy verbatim — the subtype enum is the cross-phase contract surface; both phase docs hold mutation rights jointly.

#### Combined sections

**Context.** This amendment is authored as part of the 2026-05-27 spec-coherence wave that also produced commits `0fd6bf9` (SOS-11 §15 impl entry), `4cd34b4` (ERRATA bootstrap on `docs/concepts/ERRATA.md`), and `43c45bf` (SOS-09 umbrella §12 acceptance roll-up). `ERRATA-001` / `ERRATA-002` are tracked in the bootstrap log but are unrelated to this amendment's vocabulary changes; they are named here only to anchor the amendment in the wave's commit history for future bisect.

**Reconciliation with existing SOS-03 §5/§6 content.**

- §5.1 `VectorCategory` grows from five values (`Smoke`, `Boundary`, `Stress`, `Regression`, `Diversity`) to seven (adding `Membrane` and the SOS-12-overloaded `Boundary` — see name-disambiguation note above for why the new SOS-12 surface reuses the existing `Boundary` value rather than introducing a new top-level value). Smoke's Standards-Action gate and the existing five values' registration policies are unchanged.
- §5.4 `VectorOrigin` gains no new value at this amendment; `Membrane` and dispatch-`Boundary` vectors carry `origin: "authored"` at v1 (or `origin: "regression_mined"` once the SOS-09-F / SOS-12 walkers ratify their respective miner surfaces).
- §6.2 schema is unchanged at the per-field level. The two new category surfaces add required metadata (Membrane: `channel_id`, `kind`, `dir`, `register_offset`, `protection_zone`, `subtype`; Boundary: `vector_id`, `metadata.{chart_path, trigger, expected, originating_invariant}`, `subtype`); these fields live alongside the §6.2 base set without colliding. A future SOS-03 §15 amendment ratifies the formal schema additions (typed JSON-Schema fragments per surface) when the walker outputs land their first fixtures; this amendment ratifies the vocabulary and the registration policy now so the upstream walkers (SOS-09-F, SOS-12) can emit records that this doc names.
- §6.3 vector naming convention applies to membrane-vector and boundary-vector file slugs (kebab-case lowercase ASCII derivation from a vector's `name`). The SOS-09-F `trace_key` and SOS-12 `vector_id` shapes coexist with the on-disk filename slug — they are identity fields inside the vector, not the filename.
- §6.5 structural-diff policy applies to both new surfaces per-record without modification; the field-by-field comparison walks whichever field set the surface declares, with no changes to the policy itself.

**Co-amendment obligation discharged.** Per [SOS-09 §12 (g)](./SOS-09-CONCEPTS.md#12-acceptance-checklist) and the [2026-05-27 SOS12C1 §15 entry on SOS-12-CONCEPTS.md](./SOS-12-CONCEPTS.md#change-log), this entry IS the SOS-03 §15 amendment those gates wait on. After this amendment lands, the SOS-09 umbrella's §12 (g) flips from ⏸ to ✅ at the next umbrella roll-up; the SOS-12 emitter at `tools/sos-codegen/sos12_boundary_vectors.py` emits records whose `category: "Boundary"` is now first-class in this doc's enum. No `Membrane`-category fixtures land in `conformance/vectors/membrane/` at this amendment; fixture landing rides on the SOS-09-F walker's first end-to-end run per the umbrella §12 (c) deliverable.

**INV-SOS-B preservation.** Both new categories operate at their declared layer (membrane vectors at the HW/SW boundary per SOS-09-F; boundary vectors at the dispatch boundary per SOS-12 §7.2). Per-layer bounded vector cost (no Cartesian replay across layers, INV-S-DISP-1) is preserved by SOS-12's emitter construction; per-channel bounded vector cost (six families per applicable channel) is preserved by SOS-09-F's §5.1 applicability matrix. Neither extension introduces unbounded vector growth at any layer.

Status: 🟢 ratified — additive extension; no prior amendment superseded; no §6.2 base field modified; no §5.2 `ConformanceLevel`, §5.3 `DiffSeverity`, or §5.4 `VectorOrigin` value modified. Two new `VectorCategory` values (`Membrane`; `Boundary` — overloading the existing legacy `Boundary` value name); ten new subtypes total (six membrane-vector subtypes + four boundary-vector subtypes); two new Standards-Action frozen-enum registration policies.

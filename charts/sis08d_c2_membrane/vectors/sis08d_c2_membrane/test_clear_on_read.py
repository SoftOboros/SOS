"""SOS-09-F emitted vectors — family=clear_on_read.
...
from vectors.families.clear_on_read import vector_class
...
def _load_plans():
    ...
    plans = []
    for p in raw["plans"]:
        if "clear_on_read" not in p["families"]:
            continue
...
def test_clear_on_read(plan):
    """Exercise the clear_on_read vector for each annotated channel."""

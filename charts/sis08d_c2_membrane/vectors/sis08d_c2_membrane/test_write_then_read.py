...
from vectors.families.write_then_read import vector_class
...
def _load_plans():
    ...
    plans = []
    for p in raw["plans"]:
        if "write_then_read" not in p["families"]:
            continue
...
def test_write_then_read(plan):
    """Exercise the write_then_read vector for each annotated channel."""

"""SOS-09-F emitted vectors — family=side_effect.
...
from vectors.families.side_effect import vector_class
...
def _load_plans():
    ...
    plans = []
    for p in raw["plans"]:
        if "side_effect" not in p["families"]:
            continue
...
def test_side_effect(plan):
    """Exercise the side_effect vector for each annotated channel."""

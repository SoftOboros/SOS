...
def _chart_with_n_clocks(n: int, *, parallel: bool = False) -> dict:
    ...
    clocks = []
    for i in range(n):
        # SOS7CLN (2026-05-25): wave-7a `kind=` attribute is REQUIRED on
        # every `<sos:clock>` element per PCDN-SOS-08-D-008 §15. Pre-7a
        # fixtures that omitted `kind=` previously relied on the E-walker's
        # `_backfill_implicit_kind` Option B path; that backfill is now
        # removed and fixtures declare `kind="rising"` explicitly.
        clocks.append({
...
class TestWave3FutureMultiClockTestbenchWiring:
    ...
    def test_byte_identity_for_single_clock_chart(self):
        ...
        chart_one_clock["sos:clock_domains"] = {
            # SOS7CLN: kind="rising" explicit (was relying on E-walker
            # _backfill_implicit_kind under wave-7b Option B).
            "sos:clock": [
    ...
    def test_invalid_sv_identifier_in_clock_name_raises(self):
        ...
        for bad in ("123clk", "clk-a", "clk a", "clk.a"):
            chart = {
                ...
                "state": [{"id": "idle"}],
                "sos:clock_domains": {
                    # SOS7CLN: kind="rising" explicit — exercises the
                    # SV-identifier validation path on the `name` itself.
                    "sos:clock": [
                        {"name": bad, "kind": "rising", "period_ns": 10},
                    ]
                },
            }
...
class TestPCDN008CleanupFixturesUpdated:
    """SOS7CLN (2026-05-25) — verify the wave-7b
    ``_backfill_implicit_kind`` Option B path has been retired and
    every chart-XML fixture in this test module now declares
    ``kind=`` explicitly on every ``<sos:clock>`` element.
    """

    def test_no_local_backfill_implicit_kind_remains(self):
        """``_backfill_implicit_kind`` MUST be removed from the
        E-walker module: with all fixtures now declaring
        ``kind="rising"``, the soft-reading path is unreachable and
        its presence would mask future regressions where a fixture
        accidentally drops ``kind=``.
        """
        assert not hasattr(sv_tb, "_backfill_implicit_kind"), (
            "SOS7CLN: `_backfill_implicit_kind` MUST be removed from "
            "`transliterate_hdl_sv_tb`; charts now declare `kind=` "
            "explicitly and the soft-reading path is unreachable."
        )

    def test_all_fixture_clock_blocks_declare_kind_explicitly(self):
        """Every ``<sos:clock>`` entry generated by the fixture
        helpers in this module MUST declare ``kind=`` explicitly.

        AST-walks the module and inspects every ``dict`` literal that
        carries both ``"name"`` and ``"period_ns"`` keys (the shape
        of a ``<sos:clock>`` fixture dict in this test file).  Any
        such dict MUST also carry a ``"kind"`` key.  Catches
        accidental backslides where a future fixture edit forgets to
        declare ``kind="rising"``.
        """
        import ast
        ...
        clock_dicts: list[ast.Dict] = []
        for node in ast.walk(module):
            if not isinstance(node, ast.Dict):
                continue
            keys = [k for k in node.keys if k is not None]
            has_name = any(_is_str_const(k, "name") for k in keys)
            has_period = any(_is_str_const(k, "period_ns") for k in keys)
            if has_name and has_period:
                clock_dicts.append(node)
        # We MUST have found at least one such dict — the helpers
        # build several across the wave-3-future + wave-7a fixture
        # regions.
        assert clock_dicts, (
            "fixture AST scan failed to find any `{name, period_ns}` "
            "clock dicts in the test module — the AST predicate may "
            "be stale relative to recent fixture additions."
        )
        for d in clock_dicts:
            keys = [k for k in d.keys if k is not None]
            has_kind = any(_is_str_const(k, "kind") for k in keys)
            assert has_kind, (
                f"SOS7CLN: fixture clock dict at line "
                f"{d.lineno} is missing the `kind=` attribute. All "
                "`<sos:clock>` elements MUST declare `kind=` "
                "explicitly per PCDN-SOS-08-D-008 §15 2026-05-25."
            )

// =============================================================================
// sos_synchronizer.sv  --  L0 N-FF clock-domain crossing synchronizer
...
//   syn_preserve) declared simultaneously via `(* ... *)` SV attribute
//   syntax; the synthesis tool picks the attribute family it recognizes
//   and ignores the others.  This makes the portable face self-contained
//   for "just give me a safe synchronizer" sites; the vendor-IP shim
//   path exists for callers who want xpm_cdc_single /
//   altera_std_synchronizer / vendor-curated alternatives.
//
...
//   `STAGES` >= 2 enforced via an elaboration-time `$fatal`; `WIDTH` >= 1
//   is enforced symmetrically.  Synchronous active-high `rst_dst` on the
//   destination domain (PCDN-A-003, INV-S-HDL-A-1) clears the entire flop
//   chain; there is no source-side reset port because the source-domain
//   signal is treated as asynchronous to this primitive.
//
...
//                  -- its flop chain is the very thing INV-S-HDL-3
//                  excludes from the formal model.  MTBF sign-off lives
//                  at `rtl/sos_synchronizer/MTBF.md` per PCDN-A-006.)
...
//                  this primitive is a pure data-crossing -- no
//                  handshake; associativity holds trivially.)
...
module sos_synchronizer #(
    // Mandatory; no default per INV-S-HDL-A-5.  >= 2 enforced below.
    parameter int STAGES,
    // Mandatory; no default per INV-S-HDL-A-5.  >= 1 enforced below.
    // Multi-bit users MUST Gray-code upstream (§6.9 caller responsibility).
    parameter int WIDTH
) (
    // Destination-domain clock.  The source-domain clock is not a port of
    // this primitive: the source-domain signal is treated as asynchronous
    // to `clk_dst` by construction (this is THE CDC primitive -- the whole
    // point is that the source-domain timing is not modelled here).
    input  wire                 clk_dst,
    ...
    // the chain.
    output wire [WIDTH-1:0]     d_dst
);

  // ---------------------------------------------------------------------------
  // Elaboration-time validation of STAGES (>= 2 per §6.9) and WIDTH (>= 1).
  // STAGES = 1 is not a synchronizer -- it is a single sampling flop with no
  // MTBF improvement over a direct cross.  PCDN-A-006 requires the MTBF
  // justification be computed against a STAGES >= 2 chain; STAGES = 1 fails
  // elaboration to prevent silent under-spec.
  // ---------------------------------------------------------------------------
  initial begin
    if (STAGES < 2) begin
      $fatal(1, "sos_synchronizer: SOS-08-A §6.9 requires STAGES >= 2; got %0d",
             STAGES);
    end
    if (WIDTH < 1) begin
      $fatal(1, "sos_synchronizer: SOS-08-A §6.9 requires WIDTH >= 1; got %0d",
             WIDTH);
    end
  end

  // ---------------------------------------------------------------------------
  // Synchronizer flop chain.  All three vendor synchronizer-attribute
  // families declared simultaneously via SV `(* ... *)` attribute syntax;
  // each synthesis tool picks the attribute it recognizes and silently
  // ignores the others.  This makes the portable face self-contained for
  // the common case.
  //
  ...
  //   * Intel Quartus: altera_attribute SYNCHRONIZER_IDENTIFICATION
  //     FORCED marks the chain as a vendor-recognized synchronizer for
  //     MTBF reporting and fitter-side adjacency optimisation.
  ...
  //     resolution window).
  //
  // `sync_chain[0]` captures `d_src`; `sync_chain[STAGES-1]` drives `d_dst`.
  // ---------------------------------------------------------------------------
  (* ASYNC_REG = "TRUE" *)
  ...
  assign d_dst = sync_chain[STAGES-1];

endmodule : sos_synchronizer
